...
export default defineComponent({
  props: {
    showTotalComment: { default: false, type: Boolean },
  },
  components: {
    UserComment,
    ShowMoreComments,
  ...
  methods: {
    ...mapActions(useCommentStore, ['noticePageChange']),
    handlePageChange(value: number) {
      this.noticePageChange(value)
    },
  },
})
</script>
...
<template>
  <div class="comment-group scroll-comment-list">
    ...
    <div v-if="showTotalComment" class="comment-pagination">
      <span class="total_comment_txt">Total: {{ total }}</span>
      <el-pagination @current-change="handlePageChange" background :page-size="10" layout="prev, pager, next" :total="total" />
    </div>
    <div v-else class="comment-pagination" style="justify-content: flex-end">
      <el-pagination
